...
def filter_big_file(max_file_size):
    big_file_list = []
    xml_list = glob.glob('D:/competition/crop_dataset/train/labeltxt/*.xml')
    print(len(xml_list))
    for i, xml in enumerate(xml_list):
        if os.path.getsize(xml) > max_file_size:
            big_file_list.append(xml.split('\\')[-1])
            print(i, ':', xml)
    print('out of range files have :', len(big_file_list))

    delete_path = 'D:/competition/crop_dataset/train'
    print(big_file_list)
    for j, xm in enumerate(big_file_list):
        ...
        if os.path.exists(img_path) and os.path.exists(xml_path):
            ...
            print(j, ' ', img_name, 'did not found!')
            break
